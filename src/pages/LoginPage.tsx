...
import { Lock, User, LogIn, Loader2, Shield, UserPlus } from "lucide-react";
import { loginRemote, registerRemote, getRegistrationStatus } from "@/lib/mysql-storage";
...
const LoginPage = ({ onLogin }: Props) => {
  ...
  useEffect(() => {
    ...
    if (params.get("register") === "1") {
      setIsRegister(true);
      getRegistrationStatus().then((available) => {
    ...
    try {
      if (isRegister) {
        if (password.length < 4) {
          setError("Mot de passe min. 4 caractères");
          setLoading(false);
          return;
        }
        const data = await registerRemote(username.trim(), password);
        // Auto-login after registration
        localStorage.setItem("covinor_session", JSON.stringify({ user: data.user, token: "registered" }));
        onLogin(data.user);
      } else {
  ...
  const showRegExpired = isRegister && !checkingReg && !regAvailable;
  ...
  return (
    <div className="min-h-screen bg-background noise-bg grid-pattern flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-[400px] space-y-8 relative z-10 animate-fade-in">
          ...
          <div className="text-center space-y-2">
            ...
            <div className="flex items-center justify-center gap-2 mt-3">
              ...
              <p className="text-xs text-muted-foreground uppercase tracking-[0.2em] font-display">
                {isRegister ? "Inscription" : "Espace sécurisé"}
              </p>
        ...
        {showRegExpired ? (
          <div className="glass-card-elevated rounded-2xl p-7 space-y-4 border-glow text-center">
            <div className="text-4xl">🔒</div>
            <h3 className="font-display text-lg font-bold text-foreground">Inscription fermée</h3>
            <p className="text-sm text-muted-foreground">
              Ce lien d'inscription a déjà été utilisé ou a été désactivé par l'administrateur.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="glass-card-elevated rounded-2xl p-7 space-y-5 border-glow">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-11 bg-background/50 border-border/50 rounded-xl h-12 text-sm transition-all focus:border-primary/50 focus:bg-background/80" placeholder="••••••••" autoComplete={isRegister ? "new-password" : "current-password"} />
                </div>
              </div>
              {isRegister && (
                <p className="text-[10px] text-muted-foreground text-center">
                  Créez vos identifiants pour accéder à l'application
                </p>
              )}
              {error && (
            ...
            <Button type="submit" className="w-full rounded-xl font-display font-semibold text-sm gap-2.5 glow-primary bg-gradient-to-r from-primary to-primary/90 hover:from-primary/90 hover:to-primary transition-all shimmer" disabled={loading || checkingReg} style={{ height: '52px' }}>
